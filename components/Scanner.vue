<template>
  <span class="bg-[#f2f2f2]">
    <div
      v-if="!isValid && decodedText !== ''"
    ...
    >
      ...
      <div>
        <span class="font-medium">Set ID Unrecognized!</span> Check the set ID
        and try again.
      </div>
    </div>
...
const onDecode = (text) => {
  try {
    ...
    if (
      parsedSetID.length === 6 &&
      new RegExp("[0-9A-Fa-f]{6}$").test(parsedSetID)
    ) {
